...
abstract class Modules extends AbstractModules {
	...
	public function __construct() {
		...
		$this->template->set("product_link", $this->url->product($this->data["id"]));
		$this->template->set("prod_link", $this->url->product($this->data["product_id"]));
		$this->template->set("link_delivery", $this->url->delivery());
		$this->template->set("link_contacts", $this->url->contacts());
		$this->template->set("link_docadd", $this->url->docadds());
		$this->template->set("link_posts", $this->url->posts());
	...
	protected function setLinkSort() {
		$this->template->set("link_date_up", $this->url->sortDateUp());
		$this->template->set("link_date_down", $this->url->sortDateDown());
