...
abstract class GlobalClass {
	...
	protected function getAllid($order = false, $up = true, $count = false, $offset = false) {
		$ol = $this->getOL($order, $up, $count, $offset);
		$query = "SELECT * FROM `".$this->table_name."` $ol";
		return $this->db->select($query);
	}
	...
	public function getCountSec($section_id) {
		$query = "SELECT COUNT(`section_id`) FROM `".$this->table_name."` WHERE `section_id` = $section_id";
		return $this->db->selectCell($query);
	}
